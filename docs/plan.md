...
- フェーズ：**Advanced**（N-001〜N-029 完了）
- ブロッカー：なし
- 直近の重要決定：N-029 完了。Next を N-030〜N-032 に再編（2026-05-14）
...
1. **N-030** C-004 セッション制御の警告/強制停止を段階化
2. **N-031** C-003/C-004 の永続化レート・セッション追跡
3. **N-032** C-003/C-004 の運用可観測性（メトリクス/ログ）強化
...
- 受入条件：
  - [ ] 60分超過で警告状態を返却し、120分超過とは識別できる
  - [ ] 120分超過で強制停止状態を返却できる
  - [ ] `web/app.py` の応答が警告/停止で分岐する
  - [ ] 境界値テスト（3600/3601、7200/7201）を維持しつつ新状態を検証する
...
- 受入条件：
  - [ ] レート履歴とセッション開始時刻の永続化ストアを実装する
  - [ ] 再起動後も C-003/C-004 判定が一貫する
  - [ ] 複数ユーザー並行アクセス時に判定が安定する
  - [ ] 再現性テストを追加する
  - [ ] CI 通過（全 passed・カバレッジ 80% 以上）
- 依存：N-030
...
- 受入条件：
  - [ ] C-003/C-004 イベントを構造化ログで出力する
  - [ ] 超過回数・対象ユーザー数など主要メトリクスを追加する
  - [ ] `docs/runbook.md` に監視・一次対応手順を追記する
  - [ ] CI 通過（全 passed・カバレッジ 80% 以上）
- 依存：N-031
...
| N-030 C-004 セッション制御の警告/強制停止を段階化 | [#50](https://github.com/weimaraner69-crypto/test02/issues/50) | 5-Future | 📋 予定 | Feature |
| N-031 C-003/C-004 の永続化レート・セッション追跡 | [#51](https://github.com/weimaraner69-crypto/test02/issues/51) | 5-Future | 📋 予定 | Feature |
| N-032 C-003/C-004 の運用可観測性（メトリクス/ログ）強化 | [#52](https://github.com/weimaraner69-crypto/test02/issues/52) | 5-Future | 📋 予定 | Feature |
